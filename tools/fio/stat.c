                 ...
                 struct prediction_parv_t* prediction_parv)
{
	...
	if (cur_log) {
		...
		if (iolog->log_offset) {
			...
			so->offset = offset;
            so->prediction_parv = prediction_parv;
		}
			      ...
			      unsigned long elapsed, bool log_max)
{
	...
	if (iolog->avg_window[ddir].samples) {
		...
		__add_log_sample(iolog, val, ddir, 0, elapsed, 0, 0);
	}
               ...
               struct prediction_parv_t* prediction_parv)
{
	 ...
	 */
	if (!iolog->avg_msec) {
		__add_log_sample(iolog, val, ddir, bs, elapsed, offset, prediction_parv);
		return;
	}
...
void add_agg_sample(unsigned long val, enum fio_ddir ddir, unsigned int bs)
{
	...
	__add_log_sample(iolog, val, ddir, bs, mtime_since_genesis(), 0, NULL);
}
...
static int add_bw_samples(struct thread_data *td, struct timeval *t)
{
	...
	for (ddir = DDIR_READ; ddir < DDIR_RWDIR_CNT; ddir++) {
		...
		if (td->bw_log) {
			...
			add_log_sample(td, td->bw_log, rate, ddir, bs, 0, NULL);
		}
...
static int add_iops_samples(struct thread_data *td, struct timeval *t)
{
	...
	for (ddir = DDIR_READ; ddir < DDIR_RWDIR_CNT; ddir++) {
		...
		if (td->iops_log) {
			...
			add_log_sample(td, td->iops_log, iops, ddir, bs, 0, NULL);
		}
