...
// gather group of bitidx value (1, not 0)
static uint64_t __get_bitidx_pos_mask(struct thread_data *td)
{
    uint64_t mask = 0ULL;

    if (td->o.bitidx_1 && td->o.bitidx_1_value == 1) {
        mask |= ((1ULL << td->o.bitidx_1));
    }
    if (td->o.bitidx_2 && td->o.bitidx_2_value == 1) {
        mask |= ((1ULL << td->o.bitidx_2));
    }
    if (td->o.bitidx_3 && td->o.bitidx_3_value == 1) {
        mask |= ((1ULL << td->o.bitidx_3));
    }
    if (td->o.bitidx_4 && td->o.bitidx_4_value == 1) {
        mask |= ((1ULL << td->o.bitidx_4));
    }

    return mask;
}

static uint64_t __apply_positive_mask(struct thread_data *td,
        uint64_t b_addr, uint64_t r, uint64_t lastb, uint32_t ba)
{
    uint64_t b_res = b_addr;
    uint64_t mask_aligned = __get_bitidx_pos_mask(td) / ba;

    if(mask_aligned) {
        if ((b_addr | mask_aligned) > lastb) {
            uint64_t new_random_b_addr = (lastb - mask_aligned) * (r / (rand_max(&td->random_state) + 1.0));
            b_res = (new_random_b_addr | mask_aligned);
        } else {
            b_res = b_res | mask_aligned;
        }
    }
...
static uint64_t __apply_zero_mask(struct thread_data *td,
        uint64_t b_addr, uint32_t ba)
{
    uint64_t b_res = b_addr;

    if (td->o.bitidx_1 && td->o.bitidx_1_value == 0) {
        b_res &= ~((1ULL << td->o.bitidx_1) / ba);
    }
    if (td->o.bitidx_2 && td->o.bitidx_2_value == 0) {
        b_res &= ~((1ULL << td->o.bitidx_2) / ba);
    }
    if (td->o.bitidx_3 && td->o.bitidx_3_value == 0) {
        b_res &= ~((1ULL << td->o.bitidx_3) / ba);
    }
    if (td->o.bitidx_4 && td->o.bitidx_4_value == 0) {
        b_res &= ~((1ULL << td->o.bitidx_4) / ba);
    }

    return b_res;
}

static uint64_t __address_masking(struct thread_data *td,
        uint64_t b_addr, uint64_t r, uint64_t lastb, uint32_t ba)
{
    uint64_t b_res = b_addr;

    b_res = __apply_positive_mask(td, b_res, r, lastb, ba);
    b_res = __apply_zero_mask(td, b_res, ba);
				  ...
				  uint64_t lastb)
{
    ...
    if (td->o.bitengine) {
        // add by JS, custom bitengine
        // bit index -> td->o.bitidx
        // bit index value -> td->o.bitidx_value
        if (!strcmp(td->o.bitengine, "fix")) {
            /*assert(td->o.ba[ddir] == 4096);*/
            assert(td->o.random_generator == FIO_RAND_GEN_TAUSWORTHE ||
                    td->o.random_generator == FIO_RAND_GEN_TAUSWORTHE64);

            // fix the given index by given value
            *b = __address_masking(td, *b, r, lastb, td->o.ba[ddir]);
        } else if (!strcmp(td->o.bitengine, "flip")) {
            // only change given index and fix other part by value
            // (Note) it should call with rand!!! (use original random value)
            //      Don't want to harm original random function.
            assert(0);
            /*if (*b % 2 == 0) {*/
                /**b = td->o.bitidx_value;*/
                /**b &= !(1ULL << td->o.bitidx / td->o.ba[ddir]);*/
            /*} else {*/
                /**b = td->o.bitidx_value;*/
                /**b |= (1ULL << td->o.bitidx / td->o.ba[ddir]);*/
            /*}*/
			  ...
			  unsigned int *is_random)
{
	...
	if (!ret) {
        ...
        /*if (td->o.bitengine) {*/
            ...
            /*if (!strcmp(td->o.bitengine, "fix")) {*/
                /*// fix the given index by given value*/
                /*if (td->o.bitidx_value == 0) {*/
                    /*assert(0);*/
                    /*io_u->offset &= !(1 << td->o.bitidx);*/
                /*} else if (td->o.bitidx_value == 1) {*/
                    /*assert(td->o.bitidx == 16);*/
                    /*dprint_io_u(io_u, "[JS] before");*/
                    /*io_u->offset |= (1 << td->o.bitidx);*/
                    /*dprint_io_u(io_u, "[JS] after");*/
                /*} else {*/
                    /*assert(0);*/
                /*}*/
            /*} else if (!strcmp(td->o.bitengine, "flip")) {*/
...
void io_u_queued(struct thread_data *td, struct io_u *io_u)
{
	if (!td->o.disable_slat) {
		...
		add_slat_sample(td, io_u->ddir, slat_time, io_u->xfer_buflen,
				io_u->offset, io_u->prediction_parv);
	}
}
