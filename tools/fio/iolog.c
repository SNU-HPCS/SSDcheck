...
void flush_samples(FILE *f, void *samples, uint64_t sample_size)
{
	...
	for (i = 0; i < nr_samples; i++) {
		...
		if (!log_offset) {
			...
			fprintf(f, "%lu, %3lu, %u, %u, %12llu,\t[%6ld] "
                    "normal_lat(r:%6ld, w:%6ld), "
                    "pressure[%d], preemptive[R:%d, W:%d]"
                    "counter(buffer:[%d]%3d, interval:%3d, last_high_interval:%3ld), "
                    "time_diff(%ld) <stime:%ld, EBT:%ld>\n",
					(unsigned long) s->time,
					(unsigned long) s->val,
					io_sample_ddir(s), s->bs,
					(unsigned long long) so->offset,
                    so->prediction_parv->predicted_latency,
                    so->prediction_parv->debug_engine_info.normal_read_latency,
                    so->prediction_parv->debug_engine_info.normal_write_latency,
                    so->prediction_parv->debug_engine_info.debug_buffer_backpressure,
                    so->prediction_parv->debug_engine_info.debug_preemptive_read,
                    so->prediction_parv->debug_engine_info.debug_preemptive_write,
                    so->prediction_parv->volume_idx,
                    so->prediction_parv->debug_engine_info.debug_current_request_in_buffer,
                    so->prediction_parv->debug_engine_info.debug_current_high_interval,
                    so->prediction_parv->debug_engine_info.debug_last_high_interval,
                    (so->prediction_parv->debug_engine_info.debug_estimate_block_time - so->prediction_parv->stime),
                    so->prediction_parv->stime,
                    so->prediction_parv->debug_engine_info.debug_estimate_block_time);

            /*free(so->prediction_parv);*/
		}
	}
}
