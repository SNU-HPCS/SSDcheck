...
#undef CONFIG_CFQ_GROUP_IOSCHED
...
#define rq_latency(rq) ((struct single_latency*)(rq->elv.priv[0]))
...
#define RQ_CFQQ(rq)		(struct cfq_queue*) (profile_latency? (rq_latency(rq)->priv_0) : ((rq)->elv.priv[0]))
...
static int __cfq_forced_dispatch_cfqq(struct cfq_queue *cfqq, struct request **ret_rq)
{
	int dispatched = 0;
 ...
 */
static int cfq_forced_dispatch(struct cfq_data *cfqd, struct request **ret_rq)
{
	struct cfq_queue *cfqq;
	int dispatched = 0;
    printk(KERN_WARNING "Calling cfq_forced_dispatched!\n");

	/* Expire the timeslice of the current active queue first */
	cfq_slice_expired(cfqd, 0);
	while ((cfqq = cfq_get_next_queue_forced(cfqd)) != NULL) {
		__cfq_set_active_queue(cfqd, cfqq);
		dispatched += __cfq_forced_dispatch_cfqq(cfqq, ret_rq);
        if (dispatched >= 1)
            break;
	}
 ...
 * dispatch list.
 */
static bool cfq_dispatch_request(struct cfq_data *cfqd, struct cfq_queue *cfqq, struct request **ret_rq)
{
	struct request *rq;
...
static int cfq_dispatch_requests(struct request_queue *q, int force)
{
	...
	if (unlikely(force)) {
        int dispatch_ret = cfq_forced_dispatch(cfqd, &rq);
        if (dispatch_ret > 1) {
            printk(KERN_WARNING "CFQ is trying to dispatch more than 1 request.\n");
            in_flight += dispatch_ret;
            return dispatch_ret;
        }
        if (dispatch_ret == 1) {
            goto ssdcheck_dispatch_one;
        }
        if (dispatch_ret == 0)
            return 0;
    }
	 ...
	 */
	if (!cfq_dispatch_request(cfqd, cfqq, &rq))
		return 0;
...
new_queue:
    ...
    /* For SSDcheck!! */
    if (profile_latency) {
        rq->elv.priv[0] = iosched_profile_add(latency_log);
        rq_latency(rq)->priv_0 = cfqq;
