...
extern void add_submit_request(int devid, uint32_t opcode, uint64_t lba, uint32_t data_len,
        uint64_t stime);
extern void add_complete_request(int devid, uint32_t opcode, uint64_t lba, uint32_t data_len,
        uint64_t stime, uint64_t ctime);
#endif

#ifdef ACHIEVA_SCSI_LOGGER
struct scsi_command_ts *ts_scsi_arr = NULL;
int achieva_initialized = 0;

/*int achieva_target_devid = -1;*/
/*module_param(achieva_target_devid, int, 0644);*/
/*MODULE_PARM_DESC(achieva_target_devid, "[ACHIEVA] target device id to dump latency");*/
#endif /* ACHIEVA_SCSI_LOGGER */
...
#ifdef ACHIEVA_SCSI_LOGGER
    // allocate ts_arr
    ts_scsi_arr = kmalloc(sizeof(struct scsi_command_ts) * max_cmd,
            GFP_KERNEL);
    if (!ts_scsi_arr) {
        printk(KERN_ERR "Could not alloc ts_scsi_arr fusion\n");
    } else {
        printk(KERN_ERR "[ACHIEVA] scsi_command_ts is initialized, max_cmd:%d\n", max_cmd);
        memset(ts_scsi_arr, 0, sizeof(struct scsi_command_ts) * max_cmd);
        achieva_initialized = 1;
    }
#endif /* ACHIEVA_SCSI_LOGGER */
...
megasas_ioc_init_fusion(struct megasas_instance *instance)
{
    ...
    /*trace_printk("[ACHIEVA] <fire_cmd> 444444\n");*/
...
#else /* defined(writeq) && defined(CONFIG_64BIT) */
	unsigned long flags;
...
#ifdef ACHIEVA_SCSI_LOGGER
    /*if (achieva_target_devid == scmd->device->id) {*/
    if ((0 < scmd->device->id) &&
            (16 > scmd->device->id)) {
        sector_t lba = 0;
        uint32_t data_len = 0;
        uint32_t opcode = scmd->cmnd[0];
        ...
        // get data length
        data_len |= (scmd->cmnd[7] << 8);
        data_len |= (scmd->cmnd[8]);

        // insert data into an array
        if (unlikely(ts_scsi_arr[index].target)) {
            printk(KERN_ERR "[ACHIEVA] FUCK! <fire_cmd> devid:%d, lba:%lld, data_len:%d, opcode:%d, index:%d\n",
                    scmd->device->id, cpu_to_le64(lba), data_len, opcode, index);
            ts_scsi_arr[index].target = 0;
        } else {
            /*if (opcode == WRITE_10) {*/
                getrawmonotonic(&ts_scsi_arr[index].start);
                ts_scsi_arr[index].sector = lba;
                ts_scsi_arr[index].data_len = data_len;
                ts_scsi_arr[index].target = 1;

#ifdef ACHIEVA_LAT_MODULE
                add_submit_request(scmd->device->id, opcode, ts_scsi_arr[index].sector, ts_scsi_arr[index].data_len,
                        timespec_to_ns(&ts_scsi_arr[index].start));
#else
                trace_printk("[ACHIEVA] <fire_cmd> devid:%d, lba:%lld, data_len:%d, opcode:%d, index:%d\n",
                        scmd->device->id, cpu_to_le64(lba), data_len,  opcode, index);
#endif
            /*}*/
        }
...
#endif
    /*trace_printk("[ACHIEVA] <fire_cmd> 1111111\n");*/
...
#ifdef ACHIEVA_SCSI_LOGGER
            {
                struct scsi_cmnd *t_scmd = cmd_fusion->scmd;
                /*if (achieva_target_devid ==  t_scmd->device->id) {*/
                if ((t_scmd->device->id > 0) &&
                        (t_scmd->device->id < 16)) {
                    sector_t lba = 0;
                    uint32_t data_len = 0;
                    uint32_t opcode = t_scmd->cmnd[0];
    
                    // get lba
                    lba |= (t_scmd->cmnd[2] << 24);
                    lba |= (t_scmd->cmnd[3] << 16);
                    lba |= (t_scmd->cmnd[4] << 8);
                    lba |= (t_scmd->cmnd[5]);

                    // get data length
                    data_len |= (t_scmd->cmnd[7] << 8);
                    data_len |= (t_scmd->cmnd[8]);

                    // insert data into an array
                    /*if (opcode == WRITE_10) {*/
                        if (likely(ts_scsi_arr[smid].target)) {
                            if (likely(ts_scsi_arr[smid].sector == lba &&
                                        ts_scsi_arr[smid].data_len == data_len)) {
                                struct timespec ts_diff;
                                s64 real_lat;

                                getrawmonotonic(&ts_scsi_arr[smid].end);
                                ts_diff = timespec_sub(ts_scsi_arr[smid].end, ts_scsi_arr[smid].start);
                                real_lat = timespec_to_ns(&ts_diff);
#ifdef ACHIEVA_LAT_MODULE
                                add_complete_request(t_scmd->device->id, opcode, lba, data_len,
                                        timespec_to_ns(&ts_scsi_arr[smid].start),
                                        timespec_to_ns(&ts_scsi_arr[smid].end));
#else
                                trace_printk("[ACHIEVA] lba:%010lld, rlat:%06lld, stime:%lld\n",
                                        cpu_to_le64(lba), real_lat, timespec_to_ns(&ts_scsi_arr[smid].start));
#endif
                                ts_scsi_arr[smid].target = 0;
                            } else {
                                printk(KERN_ERR "[ACHIEVA] FUCK1 <command isr> devid:%d, lba:%lld, data_len:%d, opcode:%d, index:%d\n",
                                        t_scmd->device->id, cpu_to_le64(lba), data_len,  opcode, smid);
                            }
                        } else {
                            printk(KERN_ERR "[ACHIEVA] FUCK2 <command isr> devid:%d, lba:%lld, data_len:%d, opcode:%d, index:%d\n",
                                    t_scmd->device->id, cpu_to_le64(lba), data_len,  opcode, smid);
                        }
                    /*}*/
                }
			  ...
			  struct megasas_cmd *cmd)
{
    ...
    /*trace_printk("[ACHIEVA] <fire_cmd> 222222\n");*/
	instance->instancet->fire_cmd(instance, req_desc->u.low,
				      req_desc->u.high, instance->reg_set);
}
				...
				if (cmd_fusion->sync_cmd_idx !=
					...
					if (cmd_mfi->frame->dcmd.opcode ==
						...
						if (!req_desc) {
                            ...
                            /*trace_printk("[ACHIEVA] <fire_cmd> 3333\n");*/
							instance->instancet->
							fire_cmd(instance,
								 req_desc->
								 u.low,
